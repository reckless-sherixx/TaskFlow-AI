...
3. `scripts` - 8 edges
4. `Button()` - 8 edges
5. `streamAIResponse()` - 6 edges
6. `tailwind` - 6 edges
7. `aliases` - 6 edges
8. `streamAIResponse()` - 6 edges
9. `message()` - 6 edges
10. `message()` - 5 edges
...
Cohesion: 0.18
Nodes (14): fireLog(), LoggerOptions, LogPayload, TokenUsage, withLogger(), fetch(), message(), open() (+6 more)
...
## Knowledge Gaps
- **103 isolated node(s):** `TokenUsage`, `LogPayload`, `LoggerOptions`, `Session`, `sessions` (+98 more)
...
- **What connects `TokenUsage`, `LogPayload`, `LoggerOptions` to the rest of the system?**
